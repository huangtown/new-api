...
func TestGetRandomSatisfiedChannelExcludingWithMemoryCache(t *testing.T) {
	...
	channel, err := GetRandomSatisfiedChannelExcluding("default", "test-model", 2, nil, []int{1, 2})
	require.NoError(t, err)
	require.NotNil(t, channel)
	assert.Equal(t, 3, channel.Id)

	channel, err = GetRandomSatisfiedChannelExcluding("default", "test-model", 2, nil, []int{1, 2, 3})
	require.NoError(t, err)
...
func TestGetChannelExcludingWithoutMemoryCache(t *testing.T) {
	...
	channel, err := GetChannelExcluding("default", "test-model", 2, nil, []int{1, 2})
	require.NoError(t, err)
	require.NotNil(t, channel)
	assert.Equal(t, 3, channel.Id)

	channel, err = GetChannelExcluding("default", "test-model", 2, nil, []int{1, 2, 3})
	require.NoError(t, err)
	assert.Nil(t, channel)
}
